...
def process_batch(batch_frame_clips_tensor):
    start_time = time.time()  # 记录主线程处理开始时间
    feature = feature_model(batch_frame_clips_tensor)
    end_time = time.time()  # 记录主线程处理结束时间
    print(f"Processing batch took {end_time - start_time:.4f} seconds")  # 打印处理时间
    return feature.detach().cpu().numpy()

def collect_frames(loader, frame_buffer, batch_frame_clips):
    for i, frame in enumerate(tqdm(loader, disable=True)):
        start_time = time.time()  # 记录子线程收集开始时间

        frame = frame.cuda()  # 在这里将frame放到GPU上
        ...
        # 将 frame_clip 加入批处理队列
        batch_frame_clips.append(frame_clip)

        # 当批次满了后返回批次并清空队列
        if len(batch_frame_clips) == BATCH_SIZE:
            batch_frame_clips_tensor = torch.cat(batch_frame_clips, dim=0)  # BATCH_SIZE, C, T, H, W
            batch_frame_clips.clear()  # 清空批处理队列
            
            end_time = time.time()  # 记录子线程收集结束时间
            print(f"Collecting frames took {end_time - start_time:.4f} seconds")  # 打印收集时间
            
            yield i,batch_frame_clips_tensor  # 注意这里不进行 .cuda() 操作
...
with torch.no_grad():
    with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=args.mp):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            # 启动子线程来收集 frames
            future = executor.submit(collect_frames, loader, frame_buffer, batch_frame_clips)

            for i,batch_frame_clips_tensor in future.result():
                # 主线程在这里将 batch_frame_clips_tensor 转换到 CUDA 上
                print(f"i:{i}")
                batch_frame_clips_tensor = batch_frame_clips_tensor.cuda()

                # 主线程处理收集到的批次
                feature = process_batch(batch_frame_clips_tensor)

                if frame_feature is None:
                    frame_feature = feature
                else:
                    frame_feature = np.concatenate([frame_feature, feature], axis=0)

                print(frame_feature.shape)

            # 处理剩余的未满批次帧
            if batch_frame_clips:
                batch_frame_clips_tensor = torch.cat(batch_frame_clips, dim=0)  # 处理剩余的帧
                batch_frame_clips_tensor = batch_frame_clips_tensor.cuda()  # 转换到CUDA上
                feature = process_batch(batch_frame_clips_tensor)
