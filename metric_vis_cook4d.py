...
# 读取step_idx_description.json文件并将其转化为action2idx格式
def load_action_mappings(json_file):
    action2idx = {}
    with open(json_file, 'r') as f:
        step_descriptions = json.load(f)
    
    # 将字符串索引转换为整数，并创建完整句子描述到索引的映射
    for idx_str, description in step_descriptions.items():
        idx = int(idx_str)
        # 使用完整的句子描述作为action_name
        action_name = description.strip()
        action2idx[action_name] = idx
    
    # 添加背景类
    action2idx['background'] = 0
    
    return action2idx

# # 读取txt文件并将其转化为action2idx格式(holoassist)
...
class ActionSegmentationErrorDetectionEvaluator:
    def __init__(self, args):
        ...
        for i in tqdm(range(len(annot)), desc='Processing segments'):
            ...
            if video_id in self.data_list:
                actions = [int(action) for action in annot[i]['labels']['action']]
        ...
        txt_file = f"step_idx_description.json"  # 替换为你的txt文件路径
        ...
        # print("set_label里面的内容: ", self.set_labels)
    ...
    def micro_framewise_error_detection(self, output_list=None, threshold=None, is_visualize=False):
        ...
        for video_id in self.data_list:
            ...
            # print('123video_id: ', video_id)
            # print('result: ', results[video_id]['segments'])
            video_id = "0_" + video_id
            segments = torch.tensor(results[video_id]['segments'])
            labels = torch.tensor(results[video_id]['label'])
            scores = torch.tensor(results[video_id]['score'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='CaptainCook4D')
    parser.add_argument('--dirs', type=str, default='CaptainCook4D')
    ...
    if args.error_detection:
        if args.threshold != -100.0:
            ...
            fprs = []
            tprs = []
            for threshold in tqdm(range(0, 31), desc="Testing thresholds", unit="threshold"):
