...
class HoloAssistdataset(Dataset):
    def __init__(
        ...
        self.bg_idx = 0
        self.annotations = {}
        self.action = str(action)+"_action"

        self.feat_path = os.path.join(root_dir, 'features_10fps')

        
        with open(os.path.join(root_dir, self.split+'.txt'), 'r') as fp:
            lines = fp.readlines()
        ...
        for i in tqdm(range(len(annot)), desc='Processing segments'):
            ...
            if video_id in self.data_list:
                ...
                self.annotations[video_id] = [np.array(annot[i]['labels']['time_stamp']) * self.default_fps, 
                                              np.array(actions),
    ...
    def __getitem__(self, idx):
        ...
        time_stamps, action_labels, action_labels_error = annots
