...
def batched_nms_logits(
    ...
    if num_segs == 0:
        return torch.zeros([0, 2]),\
               torch.zeros([0,]),\
               torch.zeros([0,], dtype=cls_idxs.dtype),\
               torch.zeros([0, scores_logits.shape[1]])  # 返回空 logits

    if multiclass:
        # multiclass nms: apply nms on each class independently
        new_segs, new_scores, new_cls_idxs, new_logits = [], [], [], []
        for class_id in torch.unique(cls_idxs):
            curr_indices = torch.where(cls_idxs == class_id)[0]
            # soft_nms vs nms
            if use_soft_nms:
            ...
            # 相应地处理 scores_logits
            sorted_logits = scores_logits[curr_indices]  # 按照当前 class_id 选择 logits
            
            # fill in the class index
            new_segs.append(sorted_segs)
            new_scores.append(sorted_scores)
            new_cls_idxs.append(sorted_cls_idxs)
            new_logits.append(sorted_logits)  # 保存对应的 logits

        # cat the results
        new_segs = torch.cat(new_segs)
        new_scores = torch.cat(new_scores)
        new_cls_idxs = torch.cat(new_cls_idxs)
        new_logits = torch.cat(new_logits)  # 拼接 logits
        ...
        # 同样处理 scores_logits
        new_logits = scores_logits  # 不需要重新索引，因为是 class agnostic
    ...
    # 按照排序后的 idxs 重新排序所有结果，包括 logits
    new_segs = new_segs[idxs[:max_seg_num]]
    new_scores = new_scores[idxs[:max_seg_num]]
    new_cls_idxs = new_cls_idxs[idxs[:max_seg_num]]
    new_logits = new_logits[idxs[:max_seg_num]]  # logits 同样排序

    return new_segs, new_scores, new_cls_idxs, new_logits  # 返回 logits
